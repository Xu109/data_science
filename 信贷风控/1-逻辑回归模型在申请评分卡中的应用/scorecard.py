...
for var in var_in_model:
    ...
    if -1 not in set(testData[var1]):
        testData[var1+'_Bin'] = testData[var1].map(lambda x: v3.AssignBin(x, continous_merged_dict[var1]))
    else:
        testData[var1 + '_Bin'] = testData[var1].map(lambda x: v3.AssignBin(x, continous_merged_dict[var1],[-1]))
...
ks = v3.KS(testData, 'prob', 'y')


basePoint = 250
PDO = 200
testData['score'] = testData['prob'].map(lambda x:v3.Prob2Score(x, basePoint, PDO))
